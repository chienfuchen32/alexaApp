...
exports.handler = function(event, context, callback) {
    ...
    if (event.session.user.accessToken == undefined) {
        ...
        https.get('https://api.amazon.com/user/profile?access_token=' + event.session.user.accessToken, (res) => {
        ...
        res.on('data', (d) => {
            ...
            if(res.statusCode == 200){
                alexa.emit(':tell', 'Hello ' + data.name);
                return;
            }
            else{
                alexa.emit(':tell', 'Hello Stranger');
                return;
            }
        });
        }).on('error', (e) => {
